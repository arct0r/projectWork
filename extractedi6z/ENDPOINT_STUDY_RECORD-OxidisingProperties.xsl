...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-OxidisingProperties/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:DurationOfTest | //xt:Results | //xt:Results | //xt:Results">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'Results'">Result</xsl:when>
                    <xsl:when test="name(.) = 'DurationOfTest'">Duration of test (contact time)</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:unitCode = '4193'"> ms</xsl:when>
                    <xsl:when test="./i6:unitCode = '4099'"> mm/s</xsl:when>
                    <xsl:when test="./i6:unitCode = '1342'"> other:</xsl:when>
                    <xsl:when test="./i6:unitCode = '60468'"> g/s</xsl:when>
                </xsl:choose>
        ...
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:value = '60393'">the study does not need to be conducted for flammable gases</xsl:when>
                    <xsl:when test="./i6:value = '60394'">the study does not need to be conducted for flammable aerosols</xsl:when>
                    <xsl:when test="./i6:value = '60316'">the study does not need to be conducted for organic peroxides</xsl:when>
                    <xsl:when test="./i6:value = '60399'">the study does not need to be conducted because there are no chemical groups present in the molecule which are associated with oxidising properties and hence, the classification procedure does not need to be applied</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60397'">the study does not need to be conducted because the substance is pyrophoric</xsl:when>
                    <xsl:when test="./i6:value = '60400'">the study does not need to be conducted because the organic substance does not contain oxygen or halogen atoms and hence, the classification procedure does not need to be applied</xsl:when>
                    <xsl:when test="./i6:value = '60401'">the study does not need to be conducted because the organic substance contains oxygen or halogen atoms which are chemically bonded only to carbon or hydrogen and hence, the classification procedure does not need to be applied</xsl:when>
                    <xsl:when test="./i6:value = '60402'">the study does not need to be conducted because the inorganic substance does not contain any oxygen or halogen atoms and hence, the classification procedure does not need to be applied</xsl:when>
                    <xsl:when test="./i6:value = '60403'">the study does not need to be conducted because the substance is incapable of reacting exothermically with combustible materials</xsl:when>
                    <xsl:when test="./i6:value = '61899'">the study does not need to be conducted because the substance is a gas</xsl:when>
                    <xsl:when test="./i6:value = '60396'">the study does not need to be conducted because the substance is flammable</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:ContactWith | //xt:Parameter | //xt:RemarksOnResults | //xt:SampleTested | //xt:Parameter | //xt:RemarksOnResults | //xt:SampleTested | //xt:Parameter | //xt:RemarksOnResults | //xt:Type | //xt:InterpretationOfResults">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'ContactWith'">Contact with</xsl:when>
                    <xsl:when test="name(.) = 'SampleTested'">Sample tested</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60388'">oxidising gases</xsl:when>
                    <xsl:when test="./i6:value = '60389'">oxidising liquids</xsl:when>
                    <xsl:when test="./i6:value = '60390'">oxidising solids</xsl:when>
                    <xsl:when test="./i6:value = '60392'">oxidising properties, other</xsl:when>
                    <xsl:when test="./i6:value = '61927'">oxidising properties</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60404'">UN Manual of Tests and Criteria: Test O.1 (Test for oxidizing solids)</xsl:when>
                    <xsl:when test="./i6:value = '60409'">UN Manual of Tests and Criteria: Test O.2 (Test for oxidizing liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60410'">UN Manual of Tests and Criteria: Test O.3 (Gravimetric test for oxidizing solids)</xsl:when>
                    <xsl:when test="./i6:value = '654'">EU Method A.17 (Oxidising Properties (Solids))</xsl:when>
                    <xsl:when test="./i6:value = '656'">EU Method A.21 (Oxidising Properties (Liquids))</xsl:when>
                    <xsl:when test="./i6:value = '429'">EPA OPPTS 830.6314 (Oxidising or Reducing Action)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '4089'">monoammonium phosphate</xsl:when>
                    <xsl:when test="./i6:value = '4090'">potassium permanganate</xsl:when>
                    <xsl:when test="./i6:value = '4091'">powdered cellulose</xsl:when>
                    <xsl:when test="./i6:value = '4092'">powdered iron</xsl:when>
                    <xsl:when test="./i6:value = '4093'">powdered zinc</xsl:when>
                    <xsl:when test="./i6:value = '60414'">coefficient of oxygen equivalency (Ci)</xsl:when>
                    <xsl:when test="./i6:value = '60415'">oxidising power (OP) (for gas mixture)</xsl:when>
                    <xsl:when test="./i6:value = '60416'">content of oxidising component (mol %) (for gas mixture)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60435'">test mixture 1:1</xsl:when>
                    <xsl:when test="./i6:value = '60436'">test mixture as specified</xsl:when>
                    <xsl:when test="./i6:value = '60455'">test mixture (not specified)</xsl:when>
                    <xsl:when test="./i6:value = '60437'">reference mixture 1: 1</xsl:when>
                    <xsl:when test="./i6:value = '60438'">reference mixture as specified</xsl:when>
                    <xsl:when test="./i6:value = '60462'">reference mixture (not specified)</xsl:when>
                    <xsl:when test="./i6:value = '60439'">reference mixture with 65% nitric acid + cellulose (1:1 ratio)</xsl:when>
                    <xsl:when test="./i6:value = '60440'">reference mixture with 40% aqueous sodium chlorate + cellulose (1:1 ratio)</xsl:when>
                    <xsl:when test="./i6:value = '60441'">reference mixture with 50% perchloric acid + cellulose (1:1 ratio)</xsl:when>
                    <xsl:when test="./i6:value = '60442'">inert substance</xsl:when>
                    <xsl:when test="./i6:value = '60365'">preliminary test</xsl:when>
                    <xsl:when test="./i6:value = '60894'">mean pressure rise time</xsl:when>
                    <xsl:when test="./i6:value = '60445'">the 1:1 mixture of substance and cellulose spontaneously ignites in the preliminary test</xsl:when>
                    <xsl:when test="./i6:value = '60446'">spontaneous ignition does not occur</xsl:when>
                    <xsl:when test="./i6:value = '60447'">maximum pressure of 2070 kPa not reached</xsl:when>
                    <xsl:when test="./i6:value = '60448'">result suspected of being a false positive</xsl:when>
                    <xsl:when test="./i6:value = '60449'">1:1 sample-to-cellulose ratio</xsl:when>
                    <xsl:when test="./i6:value = '60454'">4:1 sample-to-cellulose ratio</xsl:when>
                    <xsl:when test="./i6:value = '60456'">reference: 3:7 mixture potassium bromate + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60457'">reference: 2:3 mixture potassium bromate + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60458'">reference: 3:2 mixture potassium bromate + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60459'">reference: 3:1 mixture calcium peroxide + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60460'">reference: 1:1 mixture calcium peroxide + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60461'">reference: 1:2 mixture calcium peroxide + cellulose</xsl:when>
                    <xsl:when test="./i6:value = '60463'">inert atmosphere</xsl:when>
                    <xsl:when test="./i6:value = '60464'">burning rate</xsl:when>
                    <xsl:when test="./i6:value = '60465'">burning time of reaction over a distance of 200 mm</xsl:when>
                    <xsl:when test="./i6:value = '60466'">maximum burning rate</xsl:when>
                    <xsl:when test="./i6:value = '60467'">mean burning time</xsl:when>
                    <xsl:when test="./i6:value = '60469'">vigorous reaction in the preliminary test</xsl:when>
                    <xsl:when test="./i6:value = '60470'">flash burning at the surface</xsl:when>
                    <xsl:when test="./i6:value = '60471'">flaming</xsl:when>
                    <xsl:when test="./i6:value = '60472'">sparking</xsl:when>
                    <xsl:when test="./i6:value = '60473'">fuming</xsl:when>
                    <xsl:when test="./i6:value = '60474'">slow smouldering</xsl:when>
                    <xsl:when test="./i6:value = '60475'">glowing combustion</xsl:when>
                    <xsl:when test="./i6:value = '60476'">mixture does not ignite and burn</xsl:when>
                    <xsl:when test="./i6:value = '130898'">full study report</xsl:when>
                    <xsl:when test="./i6:value = '60478'">Category 1 (oxidising gases) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60482'">Category 1 (strong oxidising liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60486'">Category 1 (strong oxidising solids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60484'">Category 2 (oxidising liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60488'">Category 2 (oxidising solids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60485'">Category 3 (oxidising liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60489'">Category 3 (oxidising solids) based on GHS criteria</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:TestResultOxidisingGases | //xt:TestResultsOxidisingLiquids | //xt:TestResultsOxidisingSolids | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'TestResultOxidisingGases'">Test results (Oxidising gases)</xsl:when>
                    <xsl:when test="name(.) = 'CrossReference'">Cross-reference</xsl:when>
                    <xsl:when test="name(.) = 'Guideline'">Test guideline</xsl:when>
                    <xsl:when test="name(.) = 'TestResultsOxidisingLiquids'">Test results (Oxidising liquids)</xsl:when>
                    <xsl:when test="name(.) = 'AttachedJustification'">Attached justification</xsl:when>
                    <xsl:when test="name(.) = 'TestResultsOxidisingSolids'">Test results (Oxidising solids)</xsl:when>
