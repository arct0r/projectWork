...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Granulometry/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:AerodynamicDiameter | //xt:GeometricStandardDeviation | //xt:Mean | //xt:Size | //xt:Distribution">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'Mean'">Mean</xsl:when>
                    <xsl:when test="name(.) = 'Size'">Size</xsl:when>
                    <xsl:when test="name(.) = 'Distribution'">Distribution</xsl:when>
                    <xsl:when test="name(.) = 'GeometricStandardDeviation'">Geometric standard deviation</xsl:when>
                    <xsl:when test="name(.) = 'AerodynamicDiameter'">Mass median aerodynamic diameter</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
    ...
    <xsl:template match="//xt:DataWaivingJustification">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60263'">the study does not need to be conducted because the substance is marketed or used in a non solid or granular form</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:TypeOfParticleTested | //xt:TypeOfDistribution | //xt:RemarksOnResults | //xt:Percentile | //xt:RemarksOnResults | //xt:No | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'TypeOfParticleTested'">Type of particle tested</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'TypeOfDistribution'">Type of distribution</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60235'">particle size distribution (granulometry)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60079'">OECD Guideline 110 (Particle Size Distribution / Fibre Length and Diameter Distributions - Method A: Particle Size Distribution (effective hydrodynamic radius)</xsl:when>
                    <xsl:when test="./i6:value = '60080'">OECD Guideline 110 (Particle Size Distribution / Fibre Length and Diameter Distributions) - Method B: Fibre Length and Diameter Distributions</xsl:when>
                    <xsl:when test="./i6:value = '4088'">EPA OPPTS 830.7520 (Particle Size, Fiber Length, and Diameter Distribution)</xsl:when>
                    <xsl:when test="./i6:value = '9151'">DIN 55992-1 (Determination of a parameter for the dust formation of pigments and extenders - Part 1: Rotation method)</xsl:when>
                    <xsl:when test="./i6:value = '60081'">ISO 13318-2 (Determination of particle size distribution by centrifugal liquid sedimentation methods - Part 2: Photocentrifuge method)</xsl:when>
                    <xsl:when test="./i6:value = '60082'">ISO 13320 (Particle size analysis - Laser diffraction methods)</xsl:when>
                    <xsl:when test="./i6:value = '60083'">ISO 21501-2 (Determination of particle size distribution - Single particle light interaction methods - Part 2: Light scattering liquid-borne particle counter)</xsl:when>
                    <xsl:when test="./i6:value = '60266'">ISO 21501-3 (Determination of particle size distribution - Single particle light interaction methods - Part 3: Light extinction liquid-borne particle counter)</xsl:when>
                    <xsl:when test="./i6:value = '1191'">OECD Guideline 110 (Particle Size Distribution / Fibre Length and Diameter Distributions)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60069'">cascade impaction</xsl:when>
                    <xsl:when test="./i6:value = '60070'">continuous drop method</xsl:when>
                    <xsl:when test="./i6:value = '60071'">determination of fibre length and diameter distributions</xsl:when>
                    <xsl:when test="./i6:value = '60072'">electrical sensing zone (e.g. Coulter) method</xsl:when>
                    <xsl:when test="./i6:value = '60073'">Laser scattering/diffraction</xsl:when>
                    <xsl:when test="./i6:value = '60074'">microscopic examination</xsl:when>
                    <xsl:when test="./i6:value = '60075'">phase Doppler anemometry</xsl:when>
                    <xsl:when test="./i6:value = '60076'">rotating drum method</xsl:when>
                    <xsl:when test="./i6:value = '60077'">sedimentation (gravitational settling)</xsl:when>
                    <xsl:when test="./i6:value = '60078'">sieving</xsl:when>
                    <xsl:when test="./i6:value = '60067'">agglomerate</xsl:when>
                    <xsl:when test="./i6:value = '60068'">aggregate</xsl:when>
                    <xsl:when test="./i6:value = '60065'">primary particle</xsl:when>
                    <xsl:when test="./i6:value = '2457'">volumetric distribution</xsl:when>
                    <xsl:when test="./i6:value = '1822'">counted distribution</xsl:when>
                    <xsl:when test="./i6:value = '9150'">mass based distribution</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:ParticleSize | //xt:ParticleSizeDistribution | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'ParticleSizeDistribution'">Particle size distribution at different passages</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'ParticleSize'">Particle size</xsl:when>
                </xsl:choose>
            </td>
            <td>
                <hr/>
