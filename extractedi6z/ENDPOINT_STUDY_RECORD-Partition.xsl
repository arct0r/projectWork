...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Partition/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:Partition | //xt:Ph">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'Partition'">Partition coefficient</xsl:when>
    ...
    <xsl:template match="//xt:DataWaivingJustification | //xt:AnalyticalMethod">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'AnalyticalMethod'">Analytical method</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:value = '60238'">the study does not need to be conducted because the substance decomposes</xsl:when>
                    <xsl:when test="./i6:value = '60241'">the study does not need to be conducted because the substance has a high surface activity</xsl:when>
                    <xsl:when test="./i6:value = '60242'">the study does not need to be conducted because the substance reacts violently during the performance of the test</xsl:when>
                    <xsl:when test="./i6:value = '60243'">the study does not need to be conducted because the substance does not dissolve in water or in octanol</xsl:when>
                    <xsl:when test="./i6:value = '1342'">other:</xsl:when>
                    <xsl:when test="./i6:value = '2831'">gas chromatography</xsl:when>
                    <xsl:when test="./i6:value = '60256'">liquid chromatography</xsl:when>
                    <xsl:when test="./i6:value = '60257'">high-performance liquid chromatography</xsl:when>
                    <xsl:when test="./i6:value = '60271'">mass spectrometry</xsl:when>
                    <xsl:when test="./i6:value = '60258'">titration method</xsl:when>
                    <xsl:when test="./i6:value = '60259'">photometric method</xsl:when>
                    <xsl:when test="./i6:value = '60260'">voltammetric method</xsl:when>
                    <xsl:when test="./i6:value = '60261'">analysis (not specified)</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:PartitionCoefficientType | //xt:Type | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'PartitionCoefficientType'">Partition coefficient type</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '5666'">partition coefficient</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1189'">OECD Guideline 107 (Partition Coefficient (n-octanol / water), Shake Flask Method)</xsl:when>
                    <xsl:when test="./i6:value = '1197'">OECD Guideline 117 (Partition Coefficient (n-octanol / water), HPLC Method)</xsl:when>
                    <xsl:when test="./i6:value = '2163'">OECD Guideline 123 (Partition Coefficient (1-Octanol / Water), Slow-Stirring Method)</xsl:when>
                    <xsl:when test="./i6:value = '60249'">EU Method A.8 (Partition Coefficient - Shake Flask Method)</xsl:when>
                    <xsl:when test="./i6:value = '60250'">EU Method A.8 (Partition Coefficient - HPLC Method)</xsl:when>
                    <xsl:when test="./i6:value = '60251'">EU Method A.23 Partition Coefficient (1-Octanol/Water): Slow-Stirring Method</xsl:when>
                    <xsl:when test="./i6:value = '430'">EPA OPPTS 830.7550 (Partition Coefficient, n-octanol / H2O, Shake Flask Method)</xsl:when>
                    <xsl:when test="./i6:value = '432'">EPA OPPTS 830.7560 (Partition Coefficient, n-octanol / H2O, Generator Column Method)</xsl:when>
                    <xsl:when test="./i6:value = '433'">EPA OPPTS 830.7570 (Partition Coefficient, n-octanol / H2O, Estimation by Liquid Chromatography)</xsl:when>
                    <xsl:when test="./i6:value = '661'">EU Method A.8 (Partition Coefficient)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60254'">slow-stirring method</xsl:when>
                    <xsl:when test="./i6:value = '4790'">flask method</xsl:when>
                    <xsl:when test="./i6:value = '4786'">shake-flask method to: flask method</xsl:when>
                    <xsl:when test="./i6:value = '4787'">HPLC method</xsl:when>
                    <xsl:when test="./i6:value = '4788'">generator column method</xsl:when>
                    <xsl:when test="./i6:value = '60255'">estimation method (solubility ratio)</xsl:when>
                    <xsl:when test="./i6:value = '62008'">calculation method (fragments)</xsl:when>
                    <xsl:when test="./i6:value = '2214'">octanol-water</xsl:when>
                    <xsl:when test="./i6:value = '123'">Pow</xsl:when>
                    <xsl:when test="./i6:value = '2043'">log Pow</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:Partcoeff | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'Partcoeff'">Partition coefficient</xsl:when>
