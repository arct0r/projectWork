...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-WaterSolubility/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:LoadingOfAqueousPhase | //xt:Temp | //xt:LoadingOfAqueousPhase | //xt:IncubationDuration">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'IncubationDuration'">Incubation duration</xsl:when>
                    <xsl:when test="name(.) = 'Temp'">Temp.</xsl:when>
                    <xsl:when test="name(.) = 'LoadingOfAqueousPhase'">Loading of aqueous phase</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
    ...
    <xsl:template match="//xt:Solubility | //xt:IncubationDuration | //xt:Ph | //xt:MeanDissolvedConc">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'IncubationDuration'">Incubation duration</xsl:when>
                    <xsl:when test="name(.) = 'MeanDissolvedConc'">Mean dissolved conc.</xsl:when>
                    <xsl:when test="name(.) = 'Solubility'">Water solubility</xsl:when>
            ...
            <td class="value">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:unitCode = '2282'"> ppb</xsl:when>
    ...
    <xsl:template match="//xt:DataWaivingJustification | //xt:AnalyticalMethod">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60264'">the study does not need to be conducted because the substance is hydrolytically unstable at pH 4, 7 and 9 (half-life less than 12 hours)</xsl:when>
                    <xsl:when test="./i6:value = '60265'">the study does not need to be conducted because the substance is readily oxidisable in water</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:ConcBasedOn | //xt:RemarksOnResults | //xt:TypeOfTest | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'TypeOfTest'">Type of test</xsl:when>
                    <xsl:when test="name(.) = 'StudyResultType'">Type of information</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '3427'">water solubility</xsl:when>
                    <xsl:when test="./i6:value = '60262'">transformation / dissolution of metals and inorganic metal compounds</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1187'">OECD Guideline 105 (Water Solubility)</xsl:when>
                    <xsl:when test="./i6:value = '660'">EU Method A.6 (Water Solubility)</xsl:when>
                    <xsl:when test="./i6:value = '434'">EPA OPPTS 830.7840 (Water Solubility)</xsl:when>
                    <xsl:when test="./i6:value = '435'">EPA OPPTS 830.7860 (Water Solubility Generator Column Method)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '4789'">column elution method</xsl:when>
                    <xsl:when test="./i6:value = '4790'">flask method</xsl:when>
                    <xsl:when test="./i6:value = '4791'">analysis</xsl:when>
                    <xsl:when test="./i6:value = '9153'">ICP-MS</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60288'">completely miscible</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60289'">screening transformation/dissolution test - sparingly soluble metal compounds</xsl:when>
                    <xsl:when test="./i6:value = '60290'">full transformation/dissolution test - metals and sparingly soluble metal compounds</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:WaterSolubility | //xt:SolubilityOfMetalIonsInAqueousMedia | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'WaterSolubility'">Water solubility</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'SolubilityOfMetalIonsInAqueousMedia'">Solubility of metal ions in aqueous media</xsl:when>
        ...
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'ElementAnalysed'">Element analysed</xsl:when>
