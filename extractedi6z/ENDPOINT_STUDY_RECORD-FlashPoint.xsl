...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-FlashPoint/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:FPoint | //xt:AtmPressure">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'FPoint'">Flash point</xsl:when>
                    <xsl:when test="name(.) = 'AtmPressure'">Atm. press.</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:unitCode = '1349'"> Pa</xsl:when>
                    <xsl:when test="./i6:unitCode = '1954'"> hPa</xsl:when>
                    <xsl:when test="./i6:unitCode = '2019'"> kPa</xsl:when>
                    <xsl:when test="./i6:unitCode = '1740'"> atm</xsl:when>
                    <xsl:when test="./i6:unitCode = '139'"> Bar</xsl:when>
                    <xsl:when test="./i6:unitCode = '2046'"> mBar</xsl:when>
                    <xsl:when test="./i6:unitCode = '2121'"> mm Hg</xsl:when>
                    <xsl:when test="./i6:unitCode = '1348'"> PSI</xsl:when>
                    <xsl:when test="./i6:unitCode = '1616'"> Torr</xsl:when>
        ...
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:value = '61900'">the study does not need to be conducted because the substance is an aerosol</xsl:when>
                    <xsl:when test="./i6:value = '60237'">the study does not need to be conducted because the substance is inorganic</xsl:when>
                    <xsl:when test="./i6:value = '61902'">the study does not need to be conducted because the substance is an inorganic liquid with no covalent bonds</xsl:when>
                    <xsl:when test="./i6:value = '61903'">the study does not need to be conducted because the flash point is only relevant to liquids and low melting point solids</xsl:when>
                    <xsl:when test="./i6:value = '61904'">the study does not need to be conducted because the substance only contains volatile organic components with flash-points above 100°C for aqueous solutions</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '61907'">the study does not need to be conducted because decomposition occurred during the melting point study</xsl:when>
                    <xsl:when test="./i6:value = '61908'">the study does not need to be conducted because some impurities have an impact on the ignition source in such a way as to distort/invalidate the results</xsl:when>
                    <xsl:when test="./i6:value = '61909'">The study does not need to be conducted because the flash point for liquid organic peroxides is only relevant in the temperature range where the product is thermally stable. Above the SADT (self-accelerating decomposition temperature) of the product flash point determination is not relevant because decomposition products are evolved</xsl:when>
                    <xsl:when test="./i6:value = '61910'">The study does not need to be conducted because the flash point for liquid self-reactive substances is only relevant in the temperature range where the product is thermally stable. Above the SADT (self-accelerating decomposition temperature) of the product flash point determination is not relevant because decomposition products are evolved</xsl:when>
                    <xsl:when test="./i6:value = '1342'">other:</xsl:when>
                    <xsl:when test="./i6:value = '61901'">The study does not need to be conducted because the substance is inorganic</xsl:when>
                    <xsl:when test="./i6:value = '61905'">The study does not need to be conducted for explosives</xsl:when>
                </xsl:choose>
                <xsl:value-of select="./i6:other "/>
            </td>
        </tr>
    </xsl:template>
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:FlashPointApparatus | //xt:RemarksOnResults | //xt:TestProcedure | //xt:Results | //xt:RemarksOnResults | //xt:Type | //xt:InterpretationOfResults">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'FlashPointApparatus'">Flash point apparatus</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60267'">flash point of flammable liquids</xsl:when>
                    <xsl:when test="./i6:value = '60268'">flash point, other</xsl:when>
                    <xsl:when test="./i6:value = '61933'">flash point</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60291'">UN Manual of Tests and Criteria: Test L.2 (Sustained combustibility test)</xsl:when>
                    <xsl:when test="./i6:value = '662'">EU Method A.9 (Flash-Point)</xsl:when>
                    <xsl:when test="./i6:value = '4086'">EPA OPPTS 830.6315 (Flammability)</xsl:when>
                    <xsl:when test="./i6:value = '60292'">ISO 1516 (Determination of flash/no flash - Closed cup equilibrium method)</xsl:when>
                    <xsl:when test="./i6:value = '60293'">ISO 1523 (Determination of flash point - Closed cup equilibrium method)</xsl:when>
                    <xsl:when test="./i6:value = '60294'">ISO 2592 (Determination of flash and fire points - Cleveland open cup method)</xsl:when>
                    <xsl:when test="./i6:value = '60295'">ISO 2719 (Determination of flash point - Pensky-Martens closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '60296'">ISO 3679 (Determination of flash point - Rapid equilibrium closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '60298'">ISO 3680 (Determination of flash/no flash - Rapid equilibrium closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '60299'">ISO 9038 (Determination of sustained combustibility of liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60300'">ISO 13736 (Petroleum products and other liquids - Determination of flash point - Abel closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '4505'">ISO No., other:</xsl:when>
                    <xsl:when test="./i6:value = '60301'">ASTM D56 (Standard test method for flash point by tag closed cup tester)</xsl:when>
                    <xsl:when test="./i6:value = '60303'">ASTM D93 (Standard test methods for flash point by pensky-martens closed cup tester)</xsl:when>
                    <xsl:when test="./i6:value = '60304'">ASTM D1310 (Standard test method for flash point and fire point of liquids by tag open-cup apparatus)</xsl:when>
                    <xsl:when test="./i6:value = '60305'">ASTM D3278 (Standard test methods for flash point of liquids by small scale closed-cup apparatus)</xsl:when>
                    <xsl:when test="./i6:value = '60306'">ASTM D3941 (Standard test method for flash point by the equilibrium method with a closed-cup apparatus)</xsl:when>
                    <xsl:when test="./i6:value = '60311'">BS 2000 Part 170</xsl:when>
                    <xsl:when test="./i6:value = '60309'">DIN 51755 (Testing of Mineral Oils and Other Combustible Liquids; Determination of Flash Point by the Closed Tester according to Abel-Pensky)</xsl:when>
                    <xsl:when test="./i6:value = '60310'">NF M07-036 (Petroleum products. Determination of flash point. Abel-pensky closed tester)</xsl:when>
                    <xsl:when test="./i6:value = '4755'">ISO 1523:2002 (Determination of flash point - Closed cup equilibrium method)</xsl:when>
                    <xsl:when test="./i6:value = '4756'">ISO 2719:2002 (Determination of flash point - Pensky-Martens closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '4757'">ISO 3679:2004 (Determination of flash point - Rapid equilibrium closed cup method)</xsl:when>
                    <xsl:when test="./i6:value = '4758'">ISO 13736:1997 (Petroleum products and other liquids - Determination of flash point - Abel closed cup method)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1802'">closed cup</xsl:when>
                    <xsl:when test="./i6:value = '2224'">equilibrium method closed cup</xsl:when>
                    <xsl:when test="./i6:value = '342'">non-equilibrium method closed cup</xsl:when>
                    <xsl:when test="./i6:value = '2225'">open cup</xsl:when>
                    <xsl:when test="./i6:value = '2238'">equilibrium method open cup</xsl:when>
                    <xsl:when test="./i6:value = '2241'">non-equilibrium method open cup</xsl:when>
                    <xsl:when test="./i6:value = '343'">equilibrium method</xsl:when>
                    <xsl:when test="./i6:value = '344'">non-equilibrium method</xsl:when>
                    <xsl:when test="./i6:value = '60318'">Abel apparatus</xsl:when>
                    <xsl:when test="./i6:value = '60319'">Abel-Pensky apparatus</xsl:when>
                    <xsl:when test="./i6:value = '60320'">Tag apparatus</xsl:when>
                    <xsl:when test="./i6:value = '60321'">Pensky-Martens apparatus</xsl:when>
                    <xsl:when test="./i6:value = '60322'">Setaflash apparatus</xsl:when>
                    <xsl:when test="./i6:value = '60323'">no flash point according to standard method</xsl:when>
                    <xsl:when test="./i6:value = '60348'">no flash point up to 100°C</xsl:when>
                    <xsl:when test="./i6:value = '60349'">no flash point - measurement carried out until boiling temperature</xsl:when>
                    <xsl:when test="./i6:value = '60350'">flash point cannot be measured because of the presence of halogen hydrocarbons</xsl:when>
                    <xsl:when test="./i6:value = '60351'">not determinable, other reason:</xsl:when>
                    <xsl:when test="./i6:value = '60208'">not measured/tested</xsl:when>
                    <xsl:when test="./i6:value = '60353'">sustaining combustion, test conditions 60°C, 30 s</xsl:when>
                    <xsl:when test="./i6:value = '60354'">sustaining combustion, test conditions 60°C, 60 s</xsl:when>
                    <xsl:when test="./i6:value = '60355'">sustaining combustion, test conditions 75°C, 30 s</xsl:when>
                    <xsl:when test="./i6:value = '60356'">sustaining combustion, test conditions 75°C, 60 s</xsl:when>
                    <xsl:when test="./i6:value = '60357'">sustaining combustion</xsl:when>
                    <xsl:when test="./i6:value = '60358'">not sustaining combustion</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60360'">Category 1 (extremely flammable liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60361'">Category 2 (highly flammable liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60362'">Category 3 (flammable liquids) based on GHS criteria</xsl:when>
                    <xsl:when test="./i6:value = '60363'">Category 4 (combustible liquids) based on GHS criteria</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:FlashPoint | //xt:SustainingCombustibility | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'SustainingCombustibility'">Sustaining combustibility</xsl:when>
                    <xsl:when test="name(.) = 'FlashPoint'">Flash point</xsl:when>
        ...
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'DynamicViscosityOfTestMaterial'">Dynamic viscosity of test material</xsl:when>
