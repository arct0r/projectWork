...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Hydrolysis/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:Ph | //xt:StDev | //xt:Ph | //xt:Ph | //xt:StDev">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'StDev'">St. dev.</xsl:when>
    ...
    <xsl:template match="//xt:Duration | //xt:Temp | //xt:Temp | //xt:Temp">
        <tr>
            <td class="label">
                <xsl:choose>
    ...
    <xsl:template match="//xt:InitialConcMeasured | //xt:Recovery | //xt:Duration | //xt:HydrolysisRateConstant | //xt:HalfLife">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'HalfLife'">DT50</xsl:when>
                    <xsl:when test="name(.) = 'Duration'">Duration</xsl:when>
                    <xsl:when test="name(.) = 'InitialConcMeasured'">Initial conc. measured</xsl:when>
                    <xsl:when test="name(.) = 'HydrolysisRateConstant'">Hydrolysis rate constant</xsl:when>
                    <xsl:when test="name(.) = 'Recovery'">% Recovery</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:unitCode = '2325'"> s-1</xsl:when>
                    <xsl:when test="./i6:unitCode = '2109'"> min-1</xsl:when>
                    <xsl:when test="./i6:unitCode = '1953'"> h-1</xsl:when>
                    <xsl:when test="./i6:unitCode = '1833'"> d-1</xsl:when>
                    <xsl:when test="./i6:unitCode = '1673'"> a-1</xsl:when>
                </xsl:choose>
    ...
    <xsl:template match="//xt:DataWaivingJustification">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60766'">the study does not need to be conducted because the substance is readily biodegradable</xsl:when>
                    <xsl:when test="./i6:value = '60767'">the study does not need to be conducted because the substance is highly insoluble in water</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:Radiolabelling | //xt:AnalyticalMonitoring | //xt:PositiveControls | //xt:NegativeControls | //xt:TransformationProducts | //xt:No | //xt:RemarksOnResults | //xt:Type | //xt:RemarksOnResults | //xt:Type | //xt:ValidityCriteriaFulfilled">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'NegativeControls'">Negative controls</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '2858'">hydrolysis</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1192'">OECD Guideline 111 (Hydrolysis as a Function of pH)</xsl:when>
                    <xsl:when test="./i6:value = '4063'">EU Method C.7 (Degradation: Abiotic Degradation: Hydrolysis as a Function of pH)</xsl:when>
                    <xsl:when test="./i6:value = '4064'">EPA Guideline Subdivision N 161-1 (Hydrolysis)</xsl:when>
                    <xsl:when test="./i6:value = '4065'">EPA OPPTS 835.2110 (Hydrolysis as a Function of pH)</xsl:when>
                    <xsl:when test="./i6:value = '4066'">EPA OPPTS 835.2120 (Hydrolysis of Parent and Degradates as a Function of pH at 25°C)</xsl:when>
                    <xsl:when test="./i6:value = '4067'">EPA OPPTS 835.2130 (Hydrolysis as a Function of pH and Temperature)</xsl:when>
                    <xsl:when test="./i6:value = '570'">EPA OTS 796.3500 (Hydrolysis as a Function of pH at 25°C)</xsl:when>
                    <xsl:when test="./i6:value = '571'">EPA OTS 796.3510 (Hydrolysis as a Function of pH and Temperature)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60773'">hydrolytically stable based on preliminary test</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '15'">(pseudo-)first order (= half-life)</xsl:when>
                    <xsl:when test="./i6:value = '2328'">second order</xsl:when>
                    <xsl:when test="./i6:value = '2492'">zero order</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:DurationOfTest | //xt:IdentityTransformation | //xt:TotalRecoveryOfTestSubstance | //xt:DissipationHalfLifeOfParentCompound | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'DissipationHalfLifeOfParentCompound'">Dissipation DT50 of parent compound</xsl:when>
                    <xsl:when test="name(.) = 'DurationOfTest'">Duration of test</xsl:when>
                    <xsl:when test="name(.) = 'IdentityTransformation'">Identity of transformation products</xsl:when>
                    <xsl:when test="name(.) = 'TotalRecoveryOfTestSubstance'">Total recovery of test substance (in %)</xsl:when>
    ...
    <xsl:template match="//xt:NumberOfReplicates">
        <tr>
            <td class="label">Number of replicates</td>
            <td class="value">
                <xsl:value-of select=". "/>
            </td>
        </tr>
    </xsl:template>
    <xsl:template match="//xt:StudyPeriod | //xt:JustificationForTypeOfInformation | //xt:Remarks | //xt:VersionRemarks | //xt:MethodNoGuideline | //xt:SpecificDetailsOnTestMaterialUsedForTheStudy | //xt:SpecificDetailsOnTestMaterialUsedForTheStudyConfidential | //xt:DetailsOnSampling | //xt:DetailsOnAnalyticalMethods | //xt:Buffers | //xt:EstimationMethodIfUsed | //xt:DetailsOnTestConditions | //xt:Remarks | //xt:StatisticalMethods | //xt:OtherInformation | //xt:PreliminaryStudy | //xt:TestPerformance | //xt:DetailsOnHydrolysisAndAppearanceOfTransformationProducts | //xt:OtherKineticParameters | //xt:DetailsOnResults | //xt:ResultsWithReferenceSubstance | //xt:OtherInformation | //xt:RemarksOnResults | //xt:Remarks | //xt:Conclusions | //xt:ExecutiveSummary">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'StatisticalMethods'">Statistical methods</xsl:when>
                    <xsl:when test="name(.) = 'PreliminaryStudy'">Preliminary study</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'OtherKineticParameters'">Other kinetic parameters</xsl:when>
                    <xsl:when test="name(.) = 'EstimationMethodIfUsed'">Estimation method (if used)</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'DetailsOnHydrolysisAndAppearanceOfTransformationProducts'">Details on hydrolysis and appearance of transformation product(s)</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'Buffers'">Buffers</xsl:when>
