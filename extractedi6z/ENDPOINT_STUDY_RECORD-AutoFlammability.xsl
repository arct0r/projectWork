...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-AutoFlammability/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:VolumeSurfaceRatioM">
        <tr>
            <td class="label">Volume / surface ratio (m)</td>
            <td class="value">
                <xsl:value-of select=". "/>
            </td>
        </tr>
    </xsl:template>
    <xsl:template match="//xt:Flammability | //xt:AtmPressure | //xt:RelativeSelfIgnitionTemperature | //xt:SelfIgnitionTemperature">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'RelativeSelfIgnitionTemperature'">Relative self-ignition temperature</xsl:when>
                    <xsl:when test="name(.) = 'Flammability'">Auto-ignition temperature</xsl:when>
                    <xsl:when test="name(.) = 'SelfIgnitionTemperature'">Self-ignition temperature</xsl:when>
        ...
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:value = '60330'">the study does not need to be conducted because the substance is a liquid non flammable in air, e.g. no flash point up to 200°C</xsl:when>
                    <xsl:when test="./i6:value = '60331'">the study does not need to be conducted because the substance is a gas having no flammable range</xsl:when>
                    <xsl:when test="./i6:value = '60332'">the study does not need to be conducted because the substance is a solid having a melting point &lt;= 160°C</xsl:when>
                    <xsl:when test="./i6:value = '60334'">the study does not need to be conducted for liquid organic peroxides, because the vapours decompose during the execution of the test</xsl:when>
                    <xsl:when test="./i6:value = '60335'">the study does not need to be conducted for liquid self-reactive substances, because the vapours decompose during the execution of the test</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:RemarksOnResults | //xt:RemarksOnResults | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60325'">auto-ignition temperature (gases)</xsl:when>
                    <xsl:when test="./i6:value = '60324'">auto-ignition temperature (liquids)</xsl:when>
                    <xsl:when test="./i6:value = '60326'">relative self-ignition temperature (solids)</xsl:when>
                    <xsl:when test="./i6:value = '60327'">self-ignition of dust accumulation</xsl:when>
                    <xsl:when test="./i6:value = '60328'">auto-ignition temperature, other</xsl:when>
                    <xsl:when test="./i6:value = '61934'">auto-flammability</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '652'">EU Method A.15 (Auto-Ignition Temperature (Liquids and Gases))</xsl:when>
                    <xsl:when test="./i6:value = '653'">EU Method A.16 (Relative Self-Ignition Temperature for Solids)</xsl:when>
                    <xsl:when test="./i6:value = '60336'">AFNOR: NF T 20-037</xsl:when>
                    <xsl:when test="./i6:value = '60337'">ASTM E659 (Standard test method for autoignition temperature of liquid chemicals)</xsl:when>
                    <xsl:when test="./i6:value = '60338'">BS-4056 (Method of test for ignition temperature of gases and vapours)</xsl:when>
                    <xsl:when test="./i6:value = '60339'">BS 5501-1 (Electrical apparatus for potentially explosive atmospheres - General requirements)</xsl:when>
                    <xsl:when test="./i6:value = '60340'">BS 50014 (Electrical apparatus for potentially explosive atmospheres - General requirements)</xsl:when>
                    <xsl:when test="./i6:value = '60341'">BS 50020 (Electrical apparatus for potentially explosive atmospheres - Intrinsic safety)</xsl:when>
                    <xsl:when test="./i6:value = '60342'">BS 60079 (Explosive atmospheres. Equipment. General requirements)</xsl:when>
                    <xsl:when test="./i6:value = '60343'">DIN EN 14522, Method P (Determination of the auto ignition temperature of gases and vapours)</xsl:when>
                    <xsl:when test="./i6:value = '60344'">DIN EN 14522, Method S (Determination of the auto ignition temperature of gases and vapours)</xsl:when>
                    <xsl:when test="./i6:value = '60345'">DIN EN 15188 (Determination of the spontaneous ignition behaviour of dust accumulations)</xsl:when>
                    <xsl:when test="./i6:value = '60346'">DIN 51794 (Testing of mineral oil hydrocarbons - Determination of ignition temperature)</xsl:when>
                    <xsl:when test="./i6:value = '60347'">IEC 60079-4 (Electrical apparatus for explosive gas atmospheres. Part 4: Method of test for ignition temperature)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60386'">no self-ignition observed up to the melting point</xsl:when>
                    <xsl:when test="./i6:value = '60387'">no self ignition observed under the test conditions</xsl:when>
                    <xsl:when test="./i6:value = '60391'">not determined but test results according to UN Test N.4 are provided in section Flammability - self-heating substances</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:AutoFlammability | //xt:RelativeSelfIgnitionTemperatureSolids | //xt:SelfIgnitionTemperatureOfDustAccumulation | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'AutoFlammability'">Auto-ignition temperature (liquids / gases)</xsl:when>
                    <xsl:when test="name(.) = 'AttachedJustification'">Attached justification</xsl:when>
                    <xsl:when test="name(.) = 'RelativeSelfIgnitionTemperatureSolids'">Relative self-ignition temperature (solids)</xsl:when>
                    <xsl:when test="name(.) = 'SelfIgnitionTemperatureOfDustAccumulation'">Self-ignition temperature of dust accumulation</xsl:when>
