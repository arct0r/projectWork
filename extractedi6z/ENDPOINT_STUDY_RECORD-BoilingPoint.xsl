...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-BoilingPoint/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:BoilingPoint | //xt:Pressure | //xt:DecompositionTemp">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'BoilingPoint'">Boiling pt.</xsl:when>
                    <xsl:when test="name(.) = 'DecompositionTemp'">Decomp. temp.</xsl:when>
                    <xsl:when test="name(.) = 'Pressure'">Atm. press.</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
    ...
    <xsl:template match="//xt:DataWaivingJustification">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60224'">the study does not need to be conducted because the substance is a gas and the transition from liquid to gas, if any, does not occur close to 20°C</xsl:when>
                    <xsl:when test="./i6:value = '60225'">the study does not need to be conducted because the substance is a solid which melts above 300°C</xsl:when>
                    <xsl:when test="./i6:value = '60230'">the study does not need to be conducted because the substance is a solid which decomposes before boiling</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '60229'">the study does not need to be conducted because chemical change occurred during the melting point study</xsl:when>
        ...
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'Decomposition'">Decomposition</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '2719'">boiling point</xsl:when>
                    <xsl:when test="./i6:value = '108321'">temperature of decomposition (state purity)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '62172'">OECD Guideline 103 (Boiling Point)</xsl:when>
                    <xsl:when test="./i6:value = '62171'">OECD Guideline 103 (Boiling Point/Boiling Range)</xsl:when>
                    <xsl:when test="./i6:value = '1185'">OECD Guideline 103 (Boiling point/boiling range)</xsl:when>
                    <xsl:when test="./i6:value = '655'">EU Method A.2 (Boiling Temperature)</xsl:when>
                    <xsl:when test="./i6:value = '4084'">EPA OPPTS 830.7220 (Boiling Point / Boiling Range)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '4768'">ebulliometer</xsl:when>
                    <xsl:when test="./i6:value = '4769'">dynamic method</xsl:when>
                    <xsl:when test="./i6:value = '4770'">distillation method</xsl:when>
                    <xsl:when test="./i6:value = '4771'">method according to Siwoloboff</xsl:when>
                    <xsl:when test="./i6:value = '4772'">photocell detection</xsl:when>
                    <xsl:when test="./i6:value = '4773'">differential thermal analysis</xsl:when>
                    <xsl:when test="./i6:value = '4774'">differential scanning calorimetry</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:BoilingPoint | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'BoilingPoint'">Boiling point</xsl:when>
