...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Melting/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:MeltingPoint | //xt:Pressure | //xt:DecompTemp | //xt:SublimationTemp">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'MeltingPoint'">Melting / freezing pt.</xsl:when>
                    <xsl:when test="name(.) = 'DecompTemp'">Decomp. temp.</xsl:when>
                    <xsl:when test="name(.) = 'SublimationTemp'">Subl. temp.</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:DecompIndicator | //xt:SublimationIndicator | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'MethodType'">Type of method</xsl:when>
                    <xsl:when test="name(.) = 'DecompIndicator'">Decomposition</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'SublimationIndicator'">Sublimation</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '2911'">melting point/freezing point</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1184'">OECD Guideline 102 (Melting point / Melting Range)</xsl:when>
                    <xsl:when test="./i6:value = '646'">EU Method A.1 (Melting / Freezing Temperature)</xsl:when>
                    <xsl:when test="./i6:value = '4083'">EPA OPPTS 830.7200 (Melting Point / Melting Range)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '212'">capillary method</xsl:when>
                    <xsl:when test="./i6:value = '4774'">differential scanning calorimetry</xsl:when>
                    <xsl:when test="./i6:value = '4765'">hot stages</xsl:when>
                    <xsl:when test="./i6:value = '4766'">method to determine freezing temperature</xsl:when>
                    <xsl:when test="./i6:value = '3367'">thermal analysis</xsl:when>
                    <xsl:when test="./i6:value = '4767'">pour point</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:MeltingPoint | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'MeltingPoint'">Melting / freezing point</xsl:when>
