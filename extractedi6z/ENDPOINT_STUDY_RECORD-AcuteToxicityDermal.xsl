...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-AcuteToxicityDermal/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
        ...
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    ...
                    <xsl:when test="./i6:value = '61366'">the study need not be conducted because (i) inhalation of the substance is likely and (ii) skin contact in production and/or use is not likely and (iii) the physicochemical and toxicological properties suggest no potential for a significant rate of absorption</xsl:when>
                    <xsl:when test="./i6:value = '61367'">the study does not need to be conducted because skin contact in production and/or use is not likely</xsl:when>
                    <xsl:when test="./i6:value = '61368'">the study does not need to be conducted because the physicochemical and toxicological properties suggest no potential for a significant rate of absorption through the skin</xsl:when>
                    <xsl:when test="./i6:value = '62408'">the study does not need to be conducted because inhalation of the substance is likely</xsl:when>
                    <xsl:when test="./i6:value = '62409'">the study does not need to be conducted because the substance does not meet the criteria for classification as acute toxicity or STOT SE by the oral route and no systemic effects have been observed in in vivo studies with dermal exposure (e.g. skin irritation, skin sensitisation)</xsl:when>
                    <xsl:when test="./i6:value = '62410'">the study does not need to be conducted because the substance does not meet the criteria for classification as acute toxicity or STOT SE by the oral route and, in the absence of an in vivo study by the oral route, no systemic effects after dermal exposure are predicted on the basis of non-testing approaches (e.g. read across, QSAR studies)</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:TestType | //xt:LimitTest | //xt:Species | //xt:Strain | //xt:Sex | //xt:TypeOfCoverage | //xt:Vehicle | //xt:ControlAnimals | //xt:Sex | //xt:Endpoint | //xt:BasedOn | //xt:RemarksOnResults | //xt:BodyWeight | //xt:Type | //xt:InterpretationOfResults">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '2676'">acute toxicity: dermal</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1250'">OECD Guideline 402 (Acute Dermal Toxicity)</xsl:when>
                    <xsl:when test="./i6:value = '65620'">OECD Guideline 402 (Acute Dermal Toxicity: Fixed Dose Procedure)</xsl:when>
                    <xsl:when test="./i6:value = '406'">EPA OPP 81-2 (Acute Dermal Toxicity)</xsl:when>
                    <xsl:when test="./i6:value = '502'">EPA OPPTS 870.1200 (Acute Dermal Toxicity)</xsl:when>
                    <xsl:when test="./i6:value = '131510'">EPA OPPTS 885.0001 (Overview for Microbial Pest Control Agents)</xsl:when>
                    <xsl:when test="./i6:value = '107869'">EPA OPPTS 885.3100 (Microbial Pesticide, Acute Dermal Toxicity/Pathology)</xsl:when>
                    <xsl:when test="./i6:value = '107870'">EPA OPPTS 885.3200 (Microbial Pesticide, Acute Injection Toxicity/Pathogenicity)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '598'">EPA OTS 798.1100 (Acute Dermal Toxicity)</xsl:when>
                    <xsl:when test="./i6:value = '686'">EU Method B.3 (Acute Toxicity (Dermal))</xsl:when>
                    <xsl:when test="./i6:value = '1279'">OECD Guideline 434 (Acute Dermal Toxicity - Fixed Dose Procedure)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '3483'">guinea pig</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '61369'">no indication of skin irritation up to the relevant limit dose level</xsl:when>
    ...
    <xsl:template match="//xt:StudyPeriod | //xt:JustificationForTypeOfInformation | //xt:Remarks | //xt:VersionRemarks | //xt:MethodNoGuideline | //xt:SpecificDetailsOnTestMaterialUsedForTheStudy | //xt:SpecificDetailsOnTestMaterialUsedForTheStudyConfidential | //xt:OrganismDetails | //xt:DetailsOnDermalExposure | //xt:DurationOfExposure | //xt:Doses | //xt:DetailsOnStudyDesign | //xt:Statistics | //xt:OtherInformation | //xt:Preliminary | //xt:Mortality | //xt:GrossPathology | //xt:OtherFindings | //xt:OtherInformation | //xt:RemarksOnResults | //xt:Remarks | //xt:Conclusions | //xt:ExecutiveSummary">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'DetailsOnDermalExposure'">Details on dermal exposure</xsl:when>
