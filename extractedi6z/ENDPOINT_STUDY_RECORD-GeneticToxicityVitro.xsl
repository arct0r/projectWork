...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-GeneticToxicityVitro/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:DataWaivingJustification | //xt:PositiveControlSubstance">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'PositiveControlSubstance'">Positive control substance</xsl:when>
                    <xsl:when test="name(.) = 'DataWaivingJustification'">Justification for data waiving</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '61381'">an in vitro cytogenicity study in mammalian cells or in vitro micronucleus study does not need to be conducted because adequate data from an in vivo cytogenicity test are available</xsl:when>
                    <xsl:when test="./i6:value = '61382'">an in vitro cytogenicity study in mammalian cells or in vitro micronucleus study does not need to be conducted because the substance is known to be carcinogenic category 1A or 1B or germ cell mutagenic category 1A, 1B or 2</xsl:when>
                    <xsl:when test="./i6:value = '61383'">an in vitro gene mutation study in mammalian cells does not need to be conducted because adequate data from a reliable in vivo mammalian gene mutation test are available</xsl:when>
                    <xsl:when test="./i6:value = '61385'">an in vitro gene mutation study in mammalian cells does not need to be conducted because a positive result was found in in vitro gene mutation study in bacteria</xsl:when>
                    <xsl:when test="./i6:value = '61386'">an in vitro gene mutation study in mammalian cells does not need to be conducted because a positive result was found in in vitro cytogenicity study in mammalian cells</xsl:when>
                    <xsl:when test="./i6:value = '61387'">an in vitro gene mutation study in mammalian cells does not need to be conducted because a positive result was found in in vitro micronucleus study</xsl:when>
                    <xsl:when test="./i6:value = '1342'">other:</xsl:when>
                    <xsl:when test="./i6:value = '19'">2-acetylaminofluorene</xsl:when>
                    <xsl:when test="./i6:value = '2432'">triethylenemelamine</xsl:when>
                    <xsl:when test="./i6:value = '23'">3-methylcholanthrene</xsl:when>
                    <xsl:when test="./i6:value = '25'">4-nitroquinoline-N-oxide</xsl:when>
                    <xsl:when test="./i6:value = '26'">7,12-dimethylbenzanthracene</xsl:when>
                    <xsl:when test="./i6:value = '27'">9,10-dimethylbenzanthracene</xsl:when>
                    <xsl:when test="./i6:value = '28'">9-aminoacridine</xsl:when>
                    <xsl:when test="./i6:value = '20'">2-nitrofluorene</xsl:when>
                    <xsl:when test="./i6:value = '2374'">sodium azide</xsl:when>
                    <xsl:when test="./i6:value = '2132'">monomeric acrylamide</xsl:when>
                    <xsl:when test="./i6:value = '1102'">N-dimethylnitrosamine</xsl:when>
                    <xsl:when test="./i6:value = '1103'">N-ethyl-N-nitro-N-nitrosoguanidine</xsl:when>
                    <xsl:when test="./i6:value = '1763'">benzo(a)pyrene</xsl:when>
                    <xsl:when test="./i6:value = '1812'">congo red</xsl:when>
                    <xsl:when test="./i6:value = '64802'">colchicine</xsl:when>
                    <xsl:when test="./i6:value = '1826'">cumene hydroperoxide</xsl:when>
                    <xsl:when test="./i6:value = '1830'">cyclohexylamine</xsl:when>
                    <xsl:when test="./i6:value = '1831'">cyclophosphamide</xsl:when>
                    <xsl:when test="./i6:value = '64803'">cytosine arabinoside</xsl:when>
                    <xsl:when test="./i6:value = '1890'">ethylmethanesulphonate</xsl:when>
                    <xsl:when test="./i6:value = '1892'">ethylnitrosurea</xsl:when>
                    <xsl:when test="./i6:value = '1925'">furylfuramide</xsl:when>
                    <xsl:when test="./i6:value = '2072'">methylmethanesulfonate</xsl:when>
                    <xsl:when test="./i6:value = '2115'">mitomycin C</xsl:when>
                    <xsl:when test="./i6:value = '64804'">streptonigrin</xsl:when>
                    <xsl:when test="./i6:value = '64805'">vinblastine</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:TypeOfAssay | //xt:SpeciesStrain | //xt:AdditionalStrainCharacteristics | //xt:MetabolicActivation | //xt:NegativeControls | //xt:SolventControls | //xt:TrueNegativeControls | //xt:PositiveControls | //xt:Organism | //xt:MetActIndicator | //xt:Genotoxicity | //xt:Cytotoxicity | //xt:VehContrValid | //xt:NegContrValid | //xt:TrueNegativeControlsValidity | //xt:PosContrValid | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'TypeOfAssay'">Type of assay</xsl:when>
                    <xsl:when test="name(.) = 'Genotoxicity'">Genotoxicity</xsl:when>
                    <xsl:when test="name(.) = 'Cytotoxicity'">Cytotoxicity / choice of top concentrations</xsl:when>
                    <xsl:when test="name(.) = 'MetActIndicator'">Metabolic activation</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'MetabolicActivation'">Metabolic activation</xsl:when>
                    <xsl:when test="name(.) = 'SpeciesStrain'">Species / strain / cell type</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'TrueNegativeControlsValidity'">True negative controls validity</xsl:when>
                    <xsl:when test="name(.) = 'TrueNegativeControls'">True negative controls</xsl:when>
                    <xsl:when test="name(.) = 'NegContrValid'">Untreated negative controls validity</xsl:when>
                    <xsl:when test="name(.) = 'PositiveControls'">Positive controls</xsl:when>
                    <xsl:when test="name(.) = 'Organism'">Species / strain</xsl:when>
                    <xsl:when test="name(.) = 'Guideline'">Guideline</xsl:when>
                    <xsl:when test="name(.) = 'SolventControls'">Negative solvent / vehicle controls</xsl:when>
                    ...
                    <xsl:when test="name(.) = 'NegativeControls'">Untreated negative controls</xsl:when>
                    <xsl:when test="name(.) = 'Type'">Type</xsl:when>
                    <xsl:when test="name(.) = 'AdditionalStrainCharacteristics'">Additional strain / cell type characteristics</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '61370'">in vitro gene mutation study in bacteria</xsl:when>
                    <xsl:when test="./i6:value = '61371'">in vitro gene mutation study in mammalian cells</xsl:when>
                    <xsl:when test="./i6:value = '61373'">in vitro cytogenicity / chromosome aberration study in mammalian cells</xsl:when>
                    <xsl:when test="./i6:value = '61375'">in vitro cytogenicity / micronucleus study</xsl:when>
                    <xsl:when test="./i6:value = '61377'">in vitro DNA damage and/or repair study</xsl:when>
                    <xsl:when test="./i6:value = '61379'">in vitro transformation study in mammalian cells</xsl:when>
                    <xsl:when test="./i6:value = '61380'">genetic toxicity in vitro, other</xsl:when>
                    <xsl:when test="./i6:value = '61977'">genetic toxicity in vitro</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '61389'">EPA OPP 84-2</xsl:when>
                    <xsl:when test="./i6:value = '519'">EPA OPPTS 870.5100 - Bacterial Reverse Mutation Test (August 1998)</xsl:when>
                    <xsl:when test="./i6:value = '520'">EPA OPPTS 870.5140 - Gene Mutation in Aspergillus nidulans</xsl:when>
                    <xsl:when test="./i6:value = '523'">EPA OPPTS 870.5250 - Gene Mutation in Neurospora crassa</xsl:when>
                    <xsl:when test="./i6:value = '524'">EPA OPPTS 870.5265 (The Salmonella typhimurium Bacterial Reverse Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '526'">EPA OPPTS 870.5300 - In vitro Mammalian Cell Gene Mutation Test</xsl:when>
                    <xsl:when test="./i6:value = '527'">EPA OPPTS 870.5375 - In vitro Mammalian Chromosome Aberration Test</xsl:when>
                    <xsl:when test="./i6:value = '533'">EPA OPPTS 870.5500 - Bacterial DNA Damage or Repair Tests</xsl:when>
                    <xsl:when test="./i6:value = '534'">EPA OPPTS 870.5550 - Unscheduled DNA Synthesis in Mammalian Cells in Culture</xsl:when>
                    <xsl:when test="./i6:value = '535'">EPA OPPTS 870.5575 - Mitotic Gene Conversion in Saccharomyces cerevisiae</xsl:when>
                    <xsl:when test="./i6:value = '536'">EPA OPPTS 870.5900 - In vitro Sister Chromatid Exchange Assay</xsl:when>
                    <xsl:when test="./i6:value = '546'">EPA OPPTS 870.8800 (Morphologic transformation of cells in culture)</xsl:when>
                    <xsl:when test="./i6:value = '107873'">EPA OPPTS 885.3500 (Microbial Pesticide, Cell Culture)</xsl:when>
                    <xsl:when test="./i6:value = '554'">EPA OTS 795.2850 (Morphologic Transformation of Cells in Culture)</xsl:when>
                    <xsl:when test="./i6:value = '614'">EPA OTS 798.5100 (Escherichia coli WP2 and WP2 UVRA Reverse Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '615'">EPA OTS 798.5140 (Gene Mutation in Aspergillus nidulans)</xsl:when>
                    <xsl:when test="./i6:value = '618'">EPA OTS 798.5250 (Gene Mutation in Neurospora crassa)</xsl:when>
                    <xsl:when test="./i6:value = '619'">EPA OTS 798.5265 (The Salmonella typhimurium Bacterial Reverse Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '621'">EPA OTS 798.5300 (Detection of Gene Mutations in Somatic Cells in Culture)</xsl:when>
                    <xsl:when test="./i6:value = '622'">EPA OTS 798.5375 (In Vitro Mammalian Chromosome Aberration)</xsl:when>
                    <xsl:when test="./i6:value = '628'">EPA OTS 798.5500 (Bacterial DNA Damage or Repair Tests)</xsl:when>
                    <xsl:when test="./i6:value = '629'">EPA OTS 798.5550 (DNA Damage and Repair, Unscheduled DNA Synthesis in Mammalian Cells In Vitro)</xsl:when>
                    <xsl:when test="./i6:value = '630'">EPA OTS 798.5575 (Saccharomyces cerevisiae Mitotic Recombination Assay)</xsl:when>
                    <xsl:when test="./i6:value = '631'">EPA OTS 798.5900 (In Vitro Sister Chromatid Exchange Assay in Mammalian Cells)</xsl:when>
                    <xsl:when test="./i6:value = '666'">EU Method B.10 (Mutagenicity - In Vitro Mammalian Chromosome Aberration Test)</xsl:when>
                    <xsl:when test="./i6:value = '669'">EU Method B.13/14 (Mutagenicity - Reverse Mutation Test Using Bacteria)</xsl:when>
                    <xsl:when test="./i6:value = '670'">EU Method B.15 (Gene Mutation - Saccharomyces cerevisae)</xsl:when>
                    <xsl:when test="./i6:value = '671'">EU Method B.16 (Mitotic Recombination - Saccharomyces cerevisiae)</xsl:when>
                    <xsl:when test="./i6:value = '672'">EU Method B.17 (Mutagenicity - In Vitro Mammalian Cell Gene Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '673'">EU Method B.18 (DNA Damage and Repair - Unscheduled DNA Synthesis - Mammalian Cells In Vitro)</xsl:when>
                    <xsl:when test="./i6:value = '674'">EU Method B.19 (Sister Chromatid Exchange Assay In Vitro)</xsl:when>
                    <xsl:when test="./i6:value = '677'">EU Method B.21 (In Vitro Mammalian Cell Transformation Test)</xsl:when>
                    <xsl:when test="./i6:value = '812'">JAPAN: Guidelines for Screening Mutagenicity Testing Of Chemicals</xsl:when>
                    <xsl:when test="./i6:value = '61390'">JAPAN: Guidelines for Screening Mutagenicity Testing Of Chemicals</xsl:when>
                    <xsl:when test="./i6:value = '1287'">OECD Guideline 471 (Bacterial Reverse Mutation Assay)</xsl:when>
                    <xsl:when test="./i6:value = '1288'">OECD Guideline 472 (Genetic Toxicology: Escherichia coli, Reverse Mutation Assay)</xsl:when>
                    <xsl:when test="./i6:value = '64850'">OECD Guideline 473 (In Vitro Mammalian Chromosomal Aberration Test)</xsl:when>
                    <xsl:when test="./i6:value = '64851'">OECD Guideline 473 (In Vitro Mammalian Chromosome Aberration Test)</xsl:when>
                    <xsl:when test="./i6:value = '1289'">OECD Guideline 473 (In Vitro Mammalian Chromosome Aberration Test)</xsl:when>
                    <xsl:when test="./i6:value = '64798'">OECD Guideline 476 (In Vitro Mammalian Cell Gene Mutation Test using the Hprt and xprt genes)</xsl:when>
                    <xsl:when test="./i6:value = '64855'">OECD Guideline 476 (In Vitro Mammalian Cell Gene Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '1292'">OECD Guideline 476 (In Vitro Mammalian Cell Gene Mutation Test)</xsl:when>
                    <xsl:when test="./i6:value = '1295'">OECD Guideline 479 (Genetic Toxicology: In Vitro Sister Chromatid Exchange Assay in Mammalian Cells)</xsl:when>
                    <xsl:when test="./i6:value = '1296'">OECD Guideline 480 (Genetic Toxicology: Saccharomyces cerevisiae, Gene Mutation Assay)</xsl:when>
                    <xsl:when test="./i6:value = '1297'">OECD Guideline 481 (Genetic Toxicology: Saccharomyces cerevisiae, Mitotic Recombination Assay)</xsl:when>
                    <xsl:when test="./i6:value = '1298'">OECD Guideline 482 (Genetic Toxicology: DNA Damage and Repair, Unscheduled DNA Synthesis in Mammalian Cells In Vitro)</xsl:when>
                    <xsl:when test="./i6:value = '61388'">OECD Guideline 487 (In vitro Mammalian Cell Micronucleus Test)</xsl:when>
                    <xsl:when test="./i6:value = '61978'">OECD Guideline 490 (In Vitro Mammalian Cell Gene Mutation Tests Using the Thymidine Kinase Gene)</xsl:when>
                    <xsl:when test="./i6:value = '2246'">other guideline:</xsl:when>
                    <xsl:when test="./i6:value = '2158'">no</xsl:when>
                    <xsl:when test="./i6:value = '4179'">yes (incl. QA statement)</xsl:when>
                    <xsl:when test="./i6:value = '130'">Bacillus subtilis recombination assay</xsl:when>
                    <xsl:when test="./i6:value = '135'">bacterial forward mutation assay</xsl:when>
                    <xsl:when test="./i6:value = '136'">bacterial gene mutation assay</xsl:when>
                    <xsl:when test="./i6:value = '137'">bacterial reverse mutation assay</xsl:when>
                    <xsl:when test="./i6:value = '1555'">comet assay</xsl:when>
                    <xsl:when test="./i6:value = '317'">DNA damage and repair assay, unscheduled DNA synthesis in mammalian cells in vitro</xsl:when>
                    <xsl:when test="./i6:value = '898'">gene mutation assay in fungi</xsl:when>
                    <xsl:when test="./i6:value = '899'">in vitro mammalian cell micronucleus test</xsl:when>
                    <xsl:when test="./i6:value = '901'">in vitro mammalian chromosome aberration test</xsl:when>
                    <xsl:when test="./i6:value = '62473'">in vitro mammalian chromosome aberration test</xsl:when>
                    <xsl:when test="./i6:value = '62471'">in vitro mammalian cell gene mutation test using the Hprt and xprt genes</xsl:when>
                    <xsl:when test="./i6:value = '62472'">in vitro mammalian cell gene mutation tests using the thymidine kinase gene</xsl:when>
                    <xsl:when test="./i6:value = '900'">in vitro mammalian cell transformation assay</xsl:when>
                    <xsl:when test="./i6:value = '1080'">mitotic recombination assay with Saccharomyces cerevisiae</xsl:when>
                    <xsl:when test="./i6:value = '1049'">mammalian cell gene mutation assay</xsl:when>
                    <xsl:when test="./i6:value = '1556'">sister chromatid exchange assay in mammalian cells</xsl:when>
                    <xsl:when test="./i6:value = '318'">SOS/umu assay</xsl:when>
                    <xsl:when test="./i6:value = '1625'">transgenic BigBlue® rat cell assay for detection of gene mutation in vitro</xsl:when>
                    <xsl:when test="./i6:value = '1669'">yeast cytogenetic assay</xsl:when>
                    <xsl:when test="./i6:value = '4116'">S. typhimurium TA 1535, TA 1537, TA 98 and TA 100</xsl:when>
                    <xsl:when test="./i6:value = '9005'">S. typhimurium TA 1535, TA 1537, TA 98, TA 100 and TA 102</xsl:when>
                    <xsl:when test="./i6:value = '9006'">S. typhimurium TA 1535, TA 1537, TA 98, TA 100 and E. coli WP2</xsl:when>
                    <xsl:when test="./i6:value = '3474'">S. typhimurium TA 1535</xsl:when>
                    <xsl:when test="./i6:value = '3475'">S. typhimurium TA 1537</xsl:when>
                    <xsl:when test="./i6:value = '4117'">S. typhimurium TA 1535 pSK1002</xsl:when>
                    <xsl:when test="./i6:value = '3477'">S. typhimurium TA 97</xsl:when>
                    <xsl:when test="./i6:value = '107833'">S. typhimurium TA 97a</xsl:when>
                    <xsl:when test="./i6:value = '3478'">S. typhimurium TA 98</xsl:when>
                    <xsl:when test="./i6:value = '3472'">S. typhimurium TA 100</xsl:when>
                    <xsl:when test="./i6:value = '3476'">S. typhimurium TA 1538</xsl:when>
                    <xsl:when test="./i6:value = '3473'">S. typhimurium TA 102</xsl:when>
                    <xsl:when test="./i6:value = '3479'">S. typhimurium, other:</xsl:when>
                    <xsl:when test="./i6:value = '3468'">E. coli WP2</xsl:when>
                    <xsl:when test="./i6:value = '3469'">E. coli WP2 uvr A</xsl:when>
                    <xsl:when test="./i6:value = '3470'">E. coli WP2 uvr A pKM 101</xsl:when>
                    <xsl:when test="./i6:value = '3471'">E. coli, other:</xsl:when>
                    <xsl:when test="./i6:value = '3467'">bacteria, other:</xsl:when>
                    <xsl:when test="./i6:value = '3589'">Saccharomyces cerevisiae</xsl:when>
                    <xsl:when test="./i6:value = '3590'">yeast, other:</xsl:when>
                    <xsl:when test="./i6:value = '3494'">Chinese hamster Ovary (CHO)</xsl:when>
                    <xsl:when test="./i6:value = '64800'">Chinese hamster lung (CHL/IU)</xsl:when>
                    <xsl:when test="./i6:value = '3493'">Chinese hamster lung fibroblasts (V79)</xsl:when>
                    <xsl:when test="./i6:value = '3497'">mouse lymphoma L5178Y cells</xsl:when>
                    <xsl:when test="./i6:value = '3495'">human lymphoblastoid cells (TK6)</xsl:when>
                    <xsl:when test="./i6:value = '3496'">mammalian cell line, other:</xsl:when>
                    <xsl:when test="./i6:value = '3550'">lymphocytes:</xsl:when>
                    <xsl:when test="./i6:value = '3549'">hepatocytes:</xsl:when>
                    <xsl:when test="./i6:value = '3551'">primary culture, other:</xsl:when>
                    <xsl:when test="./i6:value = '319'">DNA polymerase A deficient</xsl:when>
                    <xsl:when test="./i6:value = '48'">acetyltransferase proficient</xsl:when>
                    <xsl:when test="./i6:value = '47'">acetyltransferase deficient</xsl:when>
                    <xsl:when test="./i6:value = '1168'">nitroreductase deficient</xsl:when>
                    <xsl:when test="./i6:value = '64801'">p53 proficiency for cell lines</xsl:when>
                    <xsl:when test="./i6:value = '2474'">with</xsl:when>
                    <xsl:when test="./i6:value = '2475'">with and without</xsl:when>
                    <xsl:when test="./i6:value = '2476'">without</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '306'">cytotoxicity</xsl:when>
                    <xsl:when test="./i6:value = '64852'">no cytotoxicity</xsl:when>
                    <xsl:when test="./i6:value = '4120'">no cytotoxicity, but tested up to precipitating concentrations</xsl:when>
                    <xsl:when test="./i6:value = '4121'">no cytotoxicity nor precipitates, but tested up to recommended limit concentrations</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '61391'">mutagenic potential (based on QSAR/QSPR prediction)</xsl:when>
                    <xsl:when test="./i6:value = '61392'">no mutagenic potential (based on QSAR/QSPR prediction)</xsl:when>
                    <xsl:when test="./i6:value = '61393'">ambiguous mutagenic potential (based on QSAR/QSPR prediction)</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:SpeciesStrain | //xt:Controls | //xt:TestRs | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'SpeciesStrain'">Species / strain</xsl:when>
    ...
    <xsl:template match="//xt:AdministrativeData | //xt:DataSource | //xt:MaterialsAndMethods | //xt:TestMaterials | //xt:Method | //xt:AnyOtherInformationOnMaterialsAndMethodsInclTables | //xt:ResultsAndDiscussion | //xt:AnyOtherInformationOnResultsInclTables | //xt:OverallRemarksAttachments | //xt:ApplicantSummaryAndConclusion">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
        ...
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'TargetGene'">Target gene</xsl:when>
                    <xsl:when test="name(.) = 'OtherInformation'"/>
                    <xsl:when test="name(.) = 'RationaleForTestConditions'">Rationale for test conditions</xsl:when>
                    <xsl:when test="name(.) = 'EvaluationCriteria'">Evaluation criteria</xsl:when>
                    <xsl:when test="name(.) = 'DetailsOnTestSystemAndConditions'">Details on test system and experimental conditions</xsl:when>
                    <xsl:when test="name(.) = 'Statistics'">Statistics</xsl:when>
                    <xsl:when test="name(.) = 'TestConcentrationsWithJustificationForTopDose'">Test concentrations with justification for top dose</xsl:when>
                    <xsl:when test="name(.) = 'MethodNoGuideline'">Principles of method if other than guideline</xsl:when>
                    <xsl:when test="name(.) = 'Vehicle'">Vehicle / solvent</xsl:when>
                    <xsl:when test="name(.) = 'Remarks'">Remarks</xsl:when>
                    <xsl:when test="name(.) = 'RemarksOnResults'">Overall remarks</xsl:when>
                    <xsl:when test="name(.) = 'DetailsOnMammalianCellLinesIfApplicable'">Details on mammalian cell type (if applicable)</xsl:when>
