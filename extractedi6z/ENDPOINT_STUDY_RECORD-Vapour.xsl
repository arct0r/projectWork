...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Vapour/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:VapourPressureAt10CBelowTransitionTemperature | //xt:VapPrAt10 | //xt:VapourPressureAt20CBelowTransitionTemperature | //xt:VapPrAt20">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'VapourPressureAt10CBelowTransitionTemperature'">Vapour pressure at 10°C below transition temperature</xsl:when>
                    <xsl:when test="name(.) = 'VapPrAt20'">Vapour pressure at 20°C above transition temperature</xsl:when>
                    <xsl:when test="name(.) = 'VapPrAt10'">Vapour pressure at 10°C above transition temperature</xsl:when>
                    <xsl:when test="name(.) = 'VapourPressureAt20CBelowTransitionTemperature'">Vapour pressure at 20°C below transition temperature</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:value-of select="./value "/>
                <xsl:choose>
                    <xsl:when test="./unitCode = '1349'"> Pa</xsl:when>
                    <xsl:when test="./unitCode = '1954'"> hPa</xsl:when>
                    <xsl:when test="./unitCode = '2019'"> kPa</xsl:when>
                    <xsl:when test="./unitCode = '1740'"> atm</xsl:when>
                    <xsl:when test="./unitCode = '139'"> Bar</xsl:when>
                    <xsl:when test="./unitCode = '2046'"> mBar</xsl:when>
                    <xsl:when test="./unitCode = '2121'"> mm Hg</xsl:when>
                    <xsl:when test="./unitCode = '1348'"> PSI</xsl:when>
                    <xsl:when test="./unitCode = '1616'"> Torr</xsl:when>
    ...
    <xsl:template match="//xt:TempQualifier | //xt:Pressure | //xt:VapourPressure | //xt:Temp">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'TempQualifier'">Temp.</xsl:when>
                    <xsl:when test="name(.) = 'Temp'">Transition temp.</xsl:when>
                    <xsl:when test="name(.) = 'VapourPressure'">Vapour pressure</xsl:when>
                    <xsl:when test="name(.) = 'Pressure'">Vapour pressure</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
    ...
    <xsl:template match="//xt:DataWaivingJustification">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60297'">the study does not need to be conducted because the melting point is above 300°C</xsl:when>
                    <xsl:when test="./i6:value = '60308'">vapour pressure testing is not required because the standard boiling point is &lt; 30 ºC, as this substance will have a vapour pressure value above the limit of measurement (i.e. 10E+05 Pa)</xsl:when>
                    <xsl:when test="./i6:value = '60227'">the study does not need to be conducted for explosives</xsl:when>
                    <xsl:when test="./i6:value = '60315'">the study does not need to be conducted because the substance is self-reactive</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:TestNo | //xt:RemarksOnResults | //xt:RemarksOnResults | //xt:Indicator | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'Indicator'">Transition / decomposition</xsl:when>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '3415'">vapour pressure</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1186'">OECD Guideline 104 (Vapour Pressure Curve)</xsl:when>
                    <xsl:when test="./i6:value = '658'">EU Method A.4 (Vapour Pressure)</xsl:when>
                    <xsl:when test="./i6:value = '1315'">EPA OPPTS 830.7950 (Vapor Pressure)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '4769'">dynamic method</xsl:when>
                    <xsl:when test="./i6:value = '4780'">static method</xsl:when>
                    <xsl:when test="./i6:value = '4781'">isoteniscope</xsl:when>
                    <xsl:when test="./i6:value = '4782'">effusion method: vapour pressure balance</xsl:when>
                    <xsl:when test="./i6:value = '4783'">effusion method: by loss of weight or by trapping vaporisate</xsl:when>
                    <xsl:when test="./i6:value = '60085'">effusion method: Knudsen cell</xsl:when>
                    <xsl:when test="./i6:value = '60086'">effusion method: isothermal thermogravimetry</xsl:when>
                    <xsl:when test="./i6:value = '4784'">gas saturation method</xsl:when>
                    <xsl:when test="./i6:value = '4785'">spinning rotor</xsl:when>
                    <xsl:when test="./i6:value = '58245'">#1</xsl:when>
                    <xsl:when test="./i6:value = '58246'">#2</xsl:when>
                    <xsl:when test="./i6:value = '58247'">#3</xsl:when>
                    <xsl:when test="./i6:value = '58248'">#4</xsl:when>
                    <xsl:when test="./i6:value = '58249'">#5</xsl:when>
                    <xsl:when test="./i6:value = '58250'">#6</xsl:when>
                    <xsl:when test="./i6:value = '58251'">#7</xsl:when>
                    <xsl:when test="./i6:value = '58252'">#8</xsl:when>
                    <xsl:when test="./i6:value = '58253'">#9</xsl:when>
                    <xsl:when test="./i6:value = '58254'">#10</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:Vapourpr | //xt:VapourPressureAt50CIfRelevantForClassificationOfGasUnderPressure | //xt:Transition | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'VapourPressureAt50CIfRelevantForClassificationOfGasUnderPressure'">Vapour pressure at 50°C (if relevant for classification of gas under pressure)</xsl:when>
                    <xsl:when test="name(.) = 'Transition'">Transition / decomposition</xsl:when>
                    <xsl:when test="name(.) = 'CrossReference'">Cross-reference</xsl:when>
                    <xsl:when test="name(.) = 'Guideline'">Test guideline</xsl:when>
                    <xsl:when test="name(.) = 'Vapourpr'">Vapour pressure</xsl:when>
