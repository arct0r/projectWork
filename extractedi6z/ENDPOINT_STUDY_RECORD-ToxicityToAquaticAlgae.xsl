...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-ToxicityToAquaticAlgae/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:ENDPOINT_STUDY_RECORD">
        <tr class="headerDocument">
            <td>Toxicity to aquatic algae and cyanobacteria</td>
            <td>
                <hr/>
            </td>
        </tr>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:AnalyticalMonitoring | //xt:Vehicle | //xt:TestOrganismsSpecies | //xt:TestType | //xt:WaterMediaType | //xt:LimitTest | //xt:ReferenceSubstancePositiveControl | //xt:Endpoint | //xt:NominalMeasured | //xt:ConcBasedOn | //xt:BasisForEffect | //xt:RemarksOnResults | //xt:Type | //xt:ValidityCriteriaFulfilled">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '108450'">effects on growth of an additional algal species</xsl:when>
                    <xsl:when test="./i6:value = '108449'">effects on growth of green algae</xsl:when>
                    <xsl:when test="./i6:value = '3384'">toxicity to aquatic algae and cyanobacteria</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1199'">OECD Guideline 201 (Alga, Growth Inhibition Test)</xsl:when>
                    <xsl:when test="./i6:value = '61952'">OECD Guideline 201 (Freshwater Alga and Cyanobacteria, Growth Inhibition Test)</xsl:when>
                    <xsl:when test="./i6:value = '108453'">American Society for Testing and Materials. ASTM E 1218-04. Standard guide for conducting 96-h toxicity tests with microalgae. In: Annual Book of ASTM Standards, Vol. 11.06, West Conshohocken, PA. Current edition approved 2004</xsl:when>
                    <xsl:when test="./i6:value = '383'">EPA OPP 122-2 (Algal Toxicity, Tiers I and II)</xsl:when>
                    <xsl:when test="./i6:value = '387'">EPA OPP 123-3 (Algal Toxicity, Tiers I and II)</xsl:when>
                    <xsl:when test="./i6:value = '497'">EPA OPPTS 850.5400 (Algal Toxicity, Tiers I and II) (January 2012)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '108544'">EPA OPPTS 885.4300  (Microbial Pesticide, Nontarget Plant Studies, Tier I)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '575'">EPA OTS 797.1050 (Algal Toxicity, Tiers I and II)</xsl:when>
                    <xsl:when test="./i6:value = '722'">EU Method C.3 (Algal Inhibition test)</xsl:when>
                    <xsl:when test="./i6:value = '861'">ISO 10253 (Water quality - Marine Algal Growth Inhibition Test with Skeletonema costatum and Phaeodactylum tricornutum)</xsl:when>
                    <xsl:when test="./i6:value = '876'">ISO 8692 (Water Quality - Fresh Water Algal Growth Inhibition Test with Scenedesmus subspicatus and Selenastrum capricornutum)</xsl:when>
                    <xsl:when test="./i6:value = '108451'">OCSPP 850.4500 (Algal Toxicity) (January 2012)</xsl:when>
                    <xsl:when test="./i6:value = '108452'">OCSPP 850.4550 (Cyanobacteria (Anabaena flos-aquae) Toxicity) (January 2012)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '53'">Agmenellum quadruplicatum</xsl:when>
                    <xsl:when test="./i6:value = '77'">Anabaena cylindrica</xsl:when>
                    <xsl:when test="./i6:value = '78'">Anabaena doloilum</xsl:when>
                    <xsl:when test="./i6:value = '79'">Anabaena flos-aquae</xsl:when>
                    <xsl:when test="./i6:value = '80'">Anabaena inaequalis</xsl:when>
                    <xsl:when test="./i6:value = '81'">Anabaena sp.</xsl:when>
                    <xsl:when test="./i6:value = '82'">Anabaena variabilis</xsl:when>
                    <xsl:when test="./i6:value = '83'">Anacystis aeruginosa</xsl:when>
                    <xsl:when test="./i6:value = '84'">Anacystis sp.</xsl:when>
                    <xsl:when test="./i6:value = '3894'">Ankistrodesmus bibraianus</xsl:when>
                    <xsl:when test="./i6:value = '94'">Ankistrodesmus falcatus</xsl:when>
                    <xsl:when test="./i6:value = '95'">Ankistrodesmus minutissimus</xsl:when>
                    <xsl:when test="./i6:value = '96'">Ankistrodesmus sp.</xsl:when>
                    <xsl:when test="./i6:value = '247'">Chlamydomonas reinhardtii</xsl:when>
                    <xsl:when test="./i6:value = '248'">Chlamydomonas sp.</xsl:when>
                    <xsl:when test="./i6:value = '249'">Chlorella emersonii</xsl:when>
                    <xsl:when test="./i6:value = '250'">Chlorella fusca</xsl:when>
                    <xsl:when test="./i6:value = '251'">Chlorella mucosa</xsl:when>
                    <xsl:when test="./i6:value = '252'">Chlorella pyrenoidosa</xsl:when>
                    <xsl:when test="./i6:value = '253'">Chlorella sp.</xsl:when>
                    <xsl:when test="./i6:value = '254'">Chlorella vulgaris</xsl:when>
                    <xsl:when test="./i6:value = '255'">Chlorococcum sp.</xsl:when>
                    <xsl:when test="./i6:value = '294'">Cyclotella cryptica</xsl:when>
                    <xsl:when test="./i6:value = '295'">Cyclotella sp.</xsl:when>
                    <xsl:when test="./i6:value = '3895'">Desmodesmus subspicatus (previous name: Scenedesmus subspicatus)</xsl:when>
                    <xsl:when test="./i6:value = '1536'">Desmodesmus subspicatus (reported as Scenedesmus subspicatus)</xsl:when>
                    <xsl:when test="./i6:value = '336'">Dictosphaerium pulchellum</xsl:when>
                    <xsl:when test="./i6:value = '337'">Dictosphaerium sp.</xsl:when>
                    <xsl:when test="./i6:value = '350'">Dunaliella bioculata</xsl:when>
                    <xsl:when test="./i6:value = '351'">Dunaliella salina</xsl:when>
                    <xsl:when test="./i6:value = '352'">Dunaliella sp.</xsl:when>
                    <xsl:when test="./i6:value = '353'">Dunaliella tertiolecta</xsl:when>
                    <xsl:when test="./i6:value = '755'">Euglena gracilis</xsl:when>
                    <xsl:when test="./i6:value = '756'">Euglena sp.</xsl:when>
                    <xsl:when test="./i6:value = '815'">Gyrodinium sp.</xsl:when>
                    <xsl:when test="./i6:value = '821'">Haematococcus pluvialis</xsl:when>
                    <xsl:when test="./i6:value = '822'">Haematococcus sp.</xsl:when>
                    <xsl:when test="./i6:value = '842'">Hormidium flaccidum</xsl:when>
                    <xsl:when test="./i6:value = '843'">Hormidium sp.</xsl:when>
                    <xsl:when test="./i6:value = '1068'">Microcystis aeruginosa</xsl:when>
                    <xsl:when test="./i6:value = '1069'">Microcystis sp.</xsl:when>
                    <xsl:when test="./i6:value = '1085'">Monoraphidium griffithii</xsl:when>
                    <xsl:when test="./i6:value = '1086'">Monoraphidium sp.</xsl:when>
                    <xsl:when test="./i6:value = '1151'">Navicula pelliculosa</xsl:when>
                    <xsl:when test="./i6:value = '1152'">Navicula seminulum</xsl:when>
                    <xsl:when test="./i6:value = '1153'">Navicula sp.</xsl:when>
                    <xsl:when test="./i6:value = '1165'">Nitella sp.</xsl:when>
                    <xsl:when test="./i6:value = '1169'">Nitscheria linearis</xsl:when>
                    <xsl:when test="./i6:value = '1170'">Nitzschia palea</xsl:when>
                    <xsl:when test="./i6:value = '1171'">Nitzschia sp.</xsl:when>
                    <xsl:when test="./i6:value = '1339'">Oscillatoria sp.</xsl:when>
                    <xsl:when test="./i6:value = '1367'">Phaeodactylum sp.</xsl:when>
                    <xsl:when test="./i6:value = '1368'">Phaeodactylum tricornutum</xsl:when>
                    <xsl:when test="./i6:value = '1372'">Phormidium tenue</xsl:when>
                    <xsl:when test="./i6:value = '3896'">Pseudokirchnerella subcapitata (reported as Raphidocelis subcapitata)</xsl:when>
                    <xsl:when test="./i6:value = '3869'">Pseudokirchnerella subcapitata (reported as Selenastrum capricornutum)</xsl:when>
                    <xsl:when test="./i6:value = '3897'">Raphidocelis subcapitata (new name: Pseudokirchnerella subcapitata)</xsl:when>
                    <xsl:when test="./i6:value = '1438'">Raphidocelis subcapitata (previous names: Pseudokirchneriella subcapitata, Selenastrum capricornutum)</xsl:when>
                    <xsl:when test="./i6:value = '1532'">Scenedesmus acutus</xsl:when>
                    <xsl:when test="./i6:value = '3898'">Scenedesmus capricornutum</xsl:when>
                    <xsl:when test="./i6:value = '1533'">Scenedesmus pannonicus</xsl:when>
                    <xsl:when test="./i6:value = '1534'">Scenedesmus quadricauda</xsl:when>
                    <xsl:when test="./i6:value = '1535'">Scenedesmus sp.</xsl:when>
                    <xsl:when test="./i6:value = '3899'">Scenedesmus subspicatus (new name: Desmodesmus subspicatus)</xsl:when>
                    <xsl:when test="./i6:value = '4158'">Selenastrum capricornutum (new name: Pseudokirchnerella subcapitata)</xsl:when>
                    <xsl:when test="./i6:value = '1544'">Selenastrum sp.</xsl:when>
                    <xsl:when test="./i6:value = '1557'">Skeletonema costatum</xsl:when>
                    <xsl:when test="./i6:value = '1558'">Skeletonema sp.</xsl:when>
                    <xsl:when test="./i6:value = '1571'">Spirulina platensis</xsl:when>
                    <xsl:when test="./i6:value = '1572'">Spirulina sp.</xsl:when>
                    <xsl:when test="./i6:value = '1579'">Stichococcus sp.</xsl:when>
                    <xsl:when test="./i6:value = '1596'">Synechococcus elongatus</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '364'">EC5</xsl:when>
                    <xsl:when test="./i6:value = '361'">EC10</xsl:when>
                    <xsl:when test="./i6:value = '7802'">EC20</xsl:when>
                    <xsl:when test="./i6:value = '365'">EC50</xsl:when>
                    <xsl:when test="./i6:value = '366'">EC90</xsl:when>
                    <xsl:when test="./i6:value = '362'">EC100</xsl:when>
                    <xsl:when test="./i6:value = '368'">EL0</xsl:when>
                    <xsl:when test="./i6:value = '372'">EL5</xsl:when>
                    <xsl:when test="./i6:value = '369'">EL10</xsl:when>
                    <xsl:when test="./i6:value = '7803'">EL20</xsl:when>
                    <xsl:when test="./i6:value = '373'">EL50</xsl:when>
                    <xsl:when test="./i6:value = '374'">EL90</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1769'">biomass</xsl:when>
                    <xsl:when test="./i6:value = '1791'">cell number</xsl:when>
                    <xsl:when test="./i6:value = '1951'">growth rate</xsl:when>
                    <xsl:when test="./i6:value = '64821'">yield</xsl:when>
