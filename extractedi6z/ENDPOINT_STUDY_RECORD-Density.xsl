...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_STUDY_RECORD-Density/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:DataWaivingJustification">
        <tr>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '60233'">the study does not need to be conducted because the substance is only stable in solution in a particular solvent and the solution density is similar to that of the solvent</xsl:when>
    ...
    <xsl:template match="//xt:Endpoint | //xt:StudyResultType | //xt:PurposeFlag | //xt:Reliability | //xt:RationalReliability | //xt:DataWaiving | //xt:ReasonPurpose | //xt:ReasonPurpose | //xt:DataAccess | //xt:DataProtectionClaimed | //xt:Qualifier | //xt:Guideline | //xt:Deviation | //xt:GLPComplianceStatement | //xt:OtherQualityAssurance | //xt:MethodType | //xt:Type | //xt:RemarksOnResults | //xt:Type">
        <tr>
            <td class="label">
                <xsl:choose>
            ...
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '2311'">relative density</xsl:when>
                    <xsl:when test="./i6:value = '2409'">tap density</xsl:when>
                    <xsl:when test="./i6:value = '1778'">bulk density</xsl:when>
                    <xsl:when test="./i6:value = '60231'">density, other</xsl:when>
                    <xsl:when test="./i6:value = '61937'">density</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '1190'">OECD Guideline 109 (Density of Liquids and Solids)</xsl:when>
                    <xsl:when test="./i6:value = '657'">EU Method A.3 (Relative Density)</xsl:when>
                    <xsl:when test="./i6:value = '4085'">EPA OPPTS 830.7300 (Density / Relative Density / Bulk Density)</xsl:when>
                    <xsl:when test="./i6:value = '60236'">CIPAC MT 186 (Bulk Density)</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '4775'">buoyancy method</xsl:when>
                    <xsl:when test="./i6:value = '60239'">buoyancy method - hydrometer</xsl:when>
                    <xsl:when test="./i6:value = '60240'">buoyancy method - hydrostatic balance</xsl:when>
                    <xsl:when test="./i6:value = '60245'">immersion ball</xsl:when>
                    <xsl:when test="./i6:value = '60246'">pour and tap (for solids)</xsl:when>
                    <xsl:when test="./i6:value = '4776'">pycnometer method</xsl:when>
                    <xsl:when test="./i6:value = '4777'">air comparison pycnometer (for solids)</xsl:when>
                    <xsl:when test="./i6:value = '4778'">oscillating densitimeter</xsl:when>
                    <xsl:when test="./i6:value = '1841'">density</xsl:when>
                    <xsl:when test="./i6:value = '2280'">pour density</xsl:when>
    ...
    <xsl:template match="//xt:AttachedJustification | //xt:CrossReference | //xt:Guideline | //xt:Density | //xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    ...
                    <xsl:when test="name(.) = 'Density'">Density</xsl:when>
