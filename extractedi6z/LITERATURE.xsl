...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/LITERATURE/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:LITERATURE">
    ...
    <xsl:template match="//xt:ReportDate">
        <tr>
    ...
    <xsl:template match="//xt:ReferenceYear">
        <tr>
    ...
    <xsl:template match="//xt:LiteratureType | //xt:StudyIDType | //xt:AttachmentType">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'AttachmentType'">Attachment type</xsl:when>
                    <xsl:when test="name(.) = 'LiteratureType'">Reference Type</xsl:when>
                    <xsl:when test="name(.) = 'StudyIDType'">Study ID type</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '1586'">study report</xsl:when>
                    <xsl:when test="./i6:value = '266'">other company data</xsl:when>
                    <xsl:when test="./i6:value = '1443'">publication</xsl:when>
                    <xsl:when test="./i6:value = '130894'">publication (copyright not owned for reproduction)</xsl:when>
                    <xsl:when test="./i6:value = '1486'">review article or handbook</xsl:when>
                    <xsl:when test="./i6:value = '1542'">secondary source</xsl:when>
                    <xsl:when test="./i6:value = '811'">grey literature</xsl:when>
                    <xsl:when test="./i6:value = '1342'">other:</xsl:when>
                    <xsl:when test="./i6:value = '130896'">Master Record Identification (MRID)</xsl:when>
                    <xsl:when test="./i6:value = '130897'">Notification of Studies (NoS) ID</xsl:when>
    ...
    <xsl:template match="//xt:StudyIdentifiers | //xt:Attachments">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'StudyIdentifiers'">Other study identifier(s)</xsl:when>
                    <xsl:when test="name(.) = 'Attachments'">Attachments</xsl:when>
                </xsl:choose>
            </td>
            <td>
                <hr/>
    ...
    <xsl:template match="//xt:GeneralInfo">
        <tr class="headerBlock">
    ...
    <xsl:template match="//xt:Name | //xt:Author | //xt:ReportNo | //xt:CompanyOwner | //xt:CompanyOwnerStudyNo">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'ReportNo'">Report number</xsl:when>
                    <xsl:when test="name(.) = 'Author'">Author</xsl:when>
                    <xsl:when test="name(.) = 'CompanyOwnerStudyNo'">Study number</xsl:when>
                    <xsl:when test="name(.) = 'CompanyOwner'">Study sponsor</xsl:when>
    ...
    <xsl:template match="//xt:Source | //xt:TestLab | //xt:StudyID | //xt:Remarks | //xt:Remarks">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'StudyID'">Study ID</xsl:when>
                    <xsl:when test="name(.) = 'TestLab'">Testing facility</xsl:when>
                    <xsl:when test="name(.) = 'Remarks'">Remarks</xsl:when>
                    <xsl:when test="name(.) = 'Source'">Bibliographic source</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:value-of select=". "/>
