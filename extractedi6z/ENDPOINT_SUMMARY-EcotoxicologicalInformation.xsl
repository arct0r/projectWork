...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:i6="http://iuclid6.echa.europa.eu/namespaces/platform-fields/v1" xmlns:i6c="http://iuclid6.echa.europa.eu/namespaces/platform-container/v1" xmlns:xt="http://iuclid6.echa.europa.eu/namespaces/ENDPOINT_SUMMARY-EcotoxicologicalInformation/7.0" version="1.0">
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="//xt:ENDPOINT_SUMMARY">
        <tr class="headerDocument">
            <td>Ecotoxicological information</td>
            <td>
                <hr/>
            </td>
        </tr>
    ...
    <xsl:template match="//xt:AssessmentFactor | //xt:AssessmentFactor | //xt:AssessmentFactor | //xt:AssessmentFactor | //xt:AssessmentFactor | //xt:AssessmentFactor | //xt:AssessmentFactor">
        <tr>
            <td class="label">Assessment factor</td>
            <td class="value">
                <xsl:value-of select=". "/>
            </td>
        </tr>
    </xsl:template>
    <xsl:template match="//xt:HazAssessConclVal | //xt:PNECFreshwaterIntermittentReleases | //xt:HazAssessConclVal | //xt:PNECMarineWaterIntermittentReleases | //xt:HazAssessConclVal | //xt:HazAssessConclVal | //xt:HazAssessConclVal | //xt:HazAssessConclVal | //xt:HazAssessConclVal | //xt:HazAssessConclVal">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'PNECMarineWaterIntermittentReleases'">PNEC marine water (intermittent releases)</xsl:when>
                    <xsl:when test="name(.) = 'HazAssessConclVal'">PNEC value</xsl:when>
                    <xsl:when test="name(.) = 'PNECFreshwaterIntermittentReleases'">PNEC freshwater (intermittent releases)</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:value-of select="./value "/>
                <xsl:choose>
                    <xsl:when test="./unitCode = '2155'"> ng/L</xsl:when>
                    ...
                    <xsl:when test="./unitCode = '2154'"> ng/kg sediment dw</xsl:when>
                    <xsl:when test="./unitCode = '2499'"> µg/kg sediment dw</xsl:when>
                    <xsl:when test="./unitCode = '2093'"> mg/kg sediment dw</xsl:when>
                    ...
                    <xsl:when test="./unitCode = '8089'"> ng/kg soil dw</xsl:when>
                    <xsl:when test="./unitCode = '8088'"> µg/kg soil dw</xsl:when>
                    <xsl:when test="./unitCode = '2096'"> mg/kg soil dw</xsl:when>
                    <xsl:when test="./unitCode = '8090'"> ng/kg food</xsl:when>
                    <xsl:when test="./unitCode = '8091'"> µg/kg food</xsl:when>
                    <xsl:when test="./unitCode = '8092'"> mg/kg food</xsl:when>
                    <xsl:when test="./unitCode = '8093'"> g/kg food</xsl:when>
    ...
    <xsl:template match="//xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl | //xt:HazAssessConcl | //xt:ExtrapolationMethod | //xt:HazAssessConcl">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'HazAssessConcl'">Hazard assessment conclusion</xsl:when>
                    <xsl:when test="name(.) = 'ExtrapolationMethod'">Extrapolation method</xsl:when>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:choose>
                    <xsl:when test="./i6:value = '8297'">PNEC aqua (freshwater)</xsl:when>
                    <xsl:when test="./i6:value = '8342'">insufficient hazard data available (further information necessary)</xsl:when>
                    <xsl:when test="./i6:value = '8119'">no data: aquatic toxicity unlikely</xsl:when>
                    ...
                    <xsl:when test="./i6:value = '5662'">assessment factor</xsl:when>
                    <xsl:when test="./i6:value = '5663'">sensitivity distribution</xsl:when>
                    <xsl:when test="./i6:value = '8298'">PNEC aqua (marine water)</xsl:when>
                    <xsl:when test="./i6:value = '8300'">PNEC STP</xsl:when>
                    <xsl:when test="./i6:value = '8120'">no emission to STP expected</xsl:when>
                    <xsl:when test="./i6:value = '8301'">PNEC sediment (freshwater)</xsl:when>
                    <xsl:when test="./i6:value = '8296'">no exposure of sediment expected</xsl:when>
                    <xsl:when test="./i6:value = '61822'">equilibrium partitioning method</xsl:when>
                    <xsl:when test="./i6:value = '8302'">PNEC sediment (marine water)</xsl:when>
                    <xsl:when test="./i6:value = '8324'">PNEC air</xsl:when>
                    <xsl:when test="./i6:value = '8325'">hazard related to composition of atmosphere identified</xsl:when>
                    <xsl:when test="./i6:value = '8303'">PNEC soil</xsl:when>
                    <xsl:when test="./i6:value = '8122'">no exposure of soil expected</xsl:when>
                    <xsl:when test="./i6:value = '8304'">PNEC oral</xsl:when>
                    <xsl:when test="./i6:value = '8201'">no potential for bioaccumulation</xsl:when>
                    <xsl:when test="./i6:value = '8323'">no potential to cause toxic effects if accumulated (in higher organisms) via the food chain</xsl:when>
                </xsl:choose>
                <xsl:value-of select="./i6:other "/>
            </td>
        </tr>
    </xsl:template>
    <xsl:template match="//xt:AttachedBackgroundMaterial">
        <tr class="headerBlock">
            <td>Attached background material</td>
            <td>
                <hr/>
            </td>
        </tr>
    ...
    <xsl:template match="//xt:AdministrativeDataSummary | //xt:HazardForAquaticOrganisms | //xt:Freshwater | //xt:MarineWater | //xt:STP | //xt:SedimentFreshWater | //xt:SedimentMarineWater | //xt:HazardForAir | //xt:Air | //xt:HazardForTerrestrialOrganisms | //xt:Soil | //xt:HazardForPredators | //xt:SecondaryPoisoning | //xt:Discussion | //xt:ConclusionOnClassification">
        <tr class="headerBlock">
            <td>
                <xsl:choose>
                    <xsl:when test="name(.) = 'MarineWater'">Marine water</xsl:when>
                    <xsl:when test="name(.) = 'SedimentMarineWater'">Sediment (marine water)</xsl:when>
                    <xsl:when test="name(.) = 'ConclusionOnClassification'">Conclusion on classification</xsl:when>
                    <xsl:when test="name(.) = 'AdministrativeDataSummary'">Administrative data</xsl:when>
                    <xsl:when test="name(.) = 'Air'">Air</xsl:when>
                    <xsl:when test="name(.) = 'Freshwater'">Freshwater</xsl:when>
                    <xsl:when test="name(.) = 'STP'">STP</xsl:when>
                    <xsl:when test="name(.) = 'SecondaryPoisoning'">Secondary poisoning</xsl:when>
                    <xsl:when test="name(.) = 'HazardForAir'">Hazard for air</xsl:when>
                    <xsl:when test="name(.) = 'HazardForAquaticOrganisms'">Hazard for aquatic organisms</xsl:when>
                    <xsl:when test="name(.) = 'Soil'">Soil</xsl:when>
                    <xsl:when test="name(.) = 'HazardForPredators'">Hazard for predators</xsl:when>
                    <xsl:when test="name(.) = 'SedimentFreshWater'">Sediment (freshwater)</xsl:when>
                    <xsl:when test="name(.) = 'HazardForTerrestrialOrganisms'">Hazard for terrestrial organisms</xsl:when>
                    <xsl:when test="name(.) = 'Discussion'">Additional information</xsl:when>
                </xsl:choose>
            </td>
            <td>
                <hr/>
    ...
    <xsl:template match="//xt:Justification | //xt:Justification | //xt:Justification | //xt:Justification | //xt:Justification | //xt:Justification | //xt:Justification | //xt:Justification | //xt:Discussion | //xt:Remarks | //xt:JustificationEnv">
        <tr>
            <td class="label">
                <xsl:choose>
                    <xsl:when test="name(.) = 'Remarks'">Remarks</xsl:when>
                    <xsl:when test="name(.) = 'Justification'">Explanation for hazard conclusion</xsl:when>
                    <xsl:when test="name(.) = 'Discussion'"/>
                    <xsl:when test="name(.) = 'JustificationEnv'"/>
                </xsl:choose>
            </td>
            <td class="value">
                <xsl:value-of select=". "/>
